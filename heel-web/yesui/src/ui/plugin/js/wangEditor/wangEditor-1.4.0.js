...
//检测jquery是否正常
if(!$){
    alert(YIUI.I18N.getString("RICHEDITOR_TEST","检测到页面没有引用jQuery，请先引用，否则wangEditor将无法使用。"));
    return;
} else if(typeof $ !== 'function' || /^\d+\.\d+\.\d+$/.test($().jquery) === false){
    alert(YIUI.I18N.getString("RICHEDITOR_CANNOT","检测到 window.jQuery 已被修改，wangEditor无法使用。"));
    return;
}
//------------------------------------定义全局变量------------------------------------
...
$.extend($E, {
    ...
    'styleConfig': {
        'fontFamilyOptions': [
            YIUI.I18N.getString("RICHEDITOR_ARIAL","宋体"), YIUI.I18N.getString("RICHEDITOR_BLACKBODY","黑体"), YIUI.I18N.getString("RICHEDITOR_REGULARSCRIPT","楷体"), 
            YIUI.I18N.getString("RICHEDITOR_OFFICIALSCRIPT","隶书"), YIUI.I18N.getString("RICHEDITOR_YOUYUAN","幼圆"), YIUI.I18N.getString("RICHEDITOR_MICROSOFTYAHEI","微软雅黑"), 
            'Arial', 'Verdana', 'Georgia', 'Times New Roman', 'Microsoft JhengHei',
            'Trebuchet MS', 'Courier New', 'Impact', 'Comic Sans MS'
        ],
        'colorOptions': {
            '#880000': YIUI.I18N.getString("RICHEDITOR_MARROON","暗红色"),
            '#800080': YIUI.I18N.getString("RICHEDITOR_PURPLE","紫色"),
            '#ff0000': YIUI.I18N.getString("RICHEDITOR_RED","红色"),
            '#ff00ff': YIUI.I18N.getString("RICHEDITOR_BRIGHTPINK","鲜粉色"),
            '#000080': YIUI.I18N.getString("RICHEDITOR_MAZARINE","深蓝色"),
            '#0000ff': YIUI.I18N.getString("RICHEDITOR_BLUE","蓝色"),
            '#00ffff': YIUI.I18N.getString("RICHEDITOR_LAKEBLUE","湖蓝色"),
            '#008080': YIUI.I18N.getString("RICHEDITOR_BLUSHGREEN","蓝绿色"),
            '#008000': YIUI.I18N.getString("RICHEDITOR_GREEN","绿色"),
            '#808000': YIUI.I18N.getString("RICHEDITOR_OLIVE","橄榄色"),
            '#00ff00': YIUI.I18N.getString("RICHEDITOR_RESEDA","浅绿色"),
            '#ffcc00': YIUI.I18N.getString("RICHEDITOR_AURANTIUS","橙黄色"),
            '#808080': YIUI.I18N.getString("RICHEDITOR_GRAY","灰色"),
            '#c0c0c0': YIUI.I18N.getString("RICHEDITOR_SILVER","银色"),
            '#000000': YIUI.I18N.getString("RICHEDITOR_BLACK","黑色"),
            '#ffffff': YIUI.I18N.getString("RICHEDITOR_WHITE","白色")
        },
        'fontsizeOptions': {
            1: '10px',
            2: '13px',
    ...
    'demoAlertConfig': {
        insertExpression: {
            title: YIUI.I18N.getString("RICHEDITOR_INSERTEXPRESSION","实际项目中，表情图标要配置到自己的服务器（速度快），也可配置多组表情，请查阅文档。\n\n\n【该弹出框在实际项目中不会出现】"),
            isAlert: false
        },
        insertImage: {
            title: YIUI.I18N.getString("RICHEDITOR_INSERTIMAGE","实际项目中，可查阅配置文件，如何配置上传本地图片（支持跨域）\n\n\n【该弹出框在实际项目中不会出现】"),
            isAlert: false
        },
        insertCode: {
            title: YIUI.I18N.getString("RICHEDITOR_INSERTCODE","实际项目中，可配置高亮代码，请查阅文档\n\n\n【该弹出框在实际项目中不会出现】"),
            isAlert: false
        }
    }
});
$.extend($E, {
    'getUploadImgComponentForCrossDomain': function(editor){
        ...
        var content =   '<p>'+YIUI.I18N.getString("RICHEDITOR_INSERTPICTURE","插入本地图片")+'</p><form id="' + formId + '" method="post" enctype="multipart/form-data" target="' + iframeId + '">'+
...
$.extend($E.fn, {
    'command': function(e, commandName, commandValue, callback){
        if( !this.currentRange() ){
            alert(YIUI.I18N.getString("RICHEDITOR_UNCHECKED","未选中编辑区，无法执行操作"));
        }else{
            var commandHook;
    ...
    */
    'wangEditor': function(options){
        if(this[0].nodeName !== 'TEXTAREA'){
            //只支持textarea
            alert(YIUI.I18N.getString("RICHEDITOR_PROMPT","wangEditor提示：请使用textarea扩展富文本框。"));
            return;
        }

        //针对一个textarea不能执行两遍 wangEditor() 事件
        if(this.data('wangEditorFlag')){
            alert(YIUI.I18N.getString("RICHEDITOR_EVENT","针对一个textarea不能执行两遍wangEditor()事件"));
            return;
        }else{
